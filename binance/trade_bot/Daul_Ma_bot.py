...
class TestStrategy(bt.Strategy):
	...
	def __init__(self):
		self.sma_8 = bt.ind.SMA(period = 5)
		self.ema_8 = bt.ind.EMA(self.sma_8,period = 5)
	...
	def next(self):
		...
		if self.live_data:
			...
			if self.crossup and not self.crossdown and not self.isbuying :
				order = exchange.create_order(self.symbol, self.type, self.longside, self.amount,price)
				self.isbuying = True
				self.isselling = False
			elif self.crossdown and not self.crossup and not self.isselling :

				order = exchange.create_order(self.symbol, self.type, self.shortside, self.amount,price)
				self.isbuying = False
				self.isselling = True
