...
class ETHStrategy(bt.Strategy):
	...
	def next(self):
		...
		if self.live_data:
			...
			# if cash - 10 > value / 3:
			self.amount = float('{:.4f}'.format(math.floor(value / 2.1 / self.data.close[0] * self.leverage * 1000.0) / 1000.0)) #取道第三位
			# elif cash - 10 < value / 3 and position_size == 0:#
			# 	self.amount = 0
			# 	self.golong = False
			# 	self.goshort = False
			'''先不買系統 重開始用'''
			if position_size > 0 and self.golong:
				self.golong = False
				print('先不買 有了')
				return
			if position_size < 0 and self.goshort:
				self.goshort = False
				print('先不賣 有了')
			...
			if position_size != 0:
				if position_size > 0 and self.data.close[0] > self.takeprofit and self.trendup:
					...
					# order = self.exchange.create_order(self.symbol, self.type, self.shortside, abs(position_size) / 4 )
					self.trendup = False
				if position_size < 0 and self.data.close[0] < self.takeprofit and self.trenddown:
					...
					# order = self.exchange.create_order(self.symbol, self.type, self.longside, abs(position_size) / 4 )# limit + price
					self.trenddown = False
			'''下面為正常買賣'''
			if self.golong:
				print('購買數量 : ',self.amount + abs(position_size))
				print('執行買入，台灣時間 : ',twt,'close : ', self.data.close[0],'sma : ',self.sma_8[0],'ema : ',self.ema_8[0])
				# order = self.exchange.create_order(self.symbol, self.type, self.longside,self.amount + abs(position_size) )# limit + price  #self.amount + position_size
				self.golong = False
			if self.goshort:
				print('販賣數量 : ',self.amount + abs(position_size))
				print('執行賣出，台灣時間 : ',twt,'close : ', self.data.close[0],'sma : ',self.sma_8[0],'ema : ',self.ema_8[0])
				# order = self.exchange.create_order(self.symbol, self.type, self.shortside, self.amount + abs(position_size))
				self.goshort = False
...
def main():
	...
	data = store.getdata(dataname='ETH/USDT', name="ETH/USDT",
