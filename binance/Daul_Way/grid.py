...
# withdraws = client.get_withdraw_history() #轉帳紀錄
info = client.get_account_snapshot(type='SPOT') #SPOT/MARGIN/FUTURES.

btcasset = info['snapshotVos'][-1]['data']['totalAssetOfBtc']
usdtasset = info['snapshotVos'][-1]['data']['balances']
print('total btc : ',btcasset,'BTC',', free usdt : ', usdtasset[-1]['free'], 'USDT')
'''
...
#         self.linregMTM = bt.ind.OLS_BetaN(plot = False) #period = 25 寫在裡面
    ...
    def next(self):
        ...
        if  self.surely_conditions_long and len(self.record_last_price) < self.gridnumber:
            ...
            self.longorder = self.order_target_percent(target=len(self.record_last_price)/self.gridnumber)
        ...
        if self.lower:
            self.gridcount = len(self.record_last_price)
            self.shortorder = self.order_target_percent(target=len(self.record_last_price)/self.gridnumber)
