...
def main(**kwargs):
    ...
    df_list = []
    # 数据起点时间
    last_datetime = dt.datetime(2021, 7, 1)  # 第一筆資料 2019,12,8
    ...
    cerebro.optstrategy(trade_pro_ETH,
                        ...
                        # rsi_filter=range(1, 10, 2),
                        k_period=range(3, 10, 1),
                        d_period=range(3, 10, 1),
                        rsi_period=range(7, 30, 4),
                        stoch_period=range(7, 30, 4),
                        # sma_filter=range(800, 2000, 100),
    ...
    global pbar
    pbar = tqdm(desc='Opt runs', leave=True, position=1, unit='run', total=total)
    cerebro.optcallback(cb=bt_opt_callback)
    back = cerebro.run(optreturn=False, maxcpus=None)
    '''write into csv file'''
    final_results_list = []
    with open('params_rsi_2021_08_07.csv', 'w', newline='') as csvfile:
        print('開始寫入')
    ...
    # while True:
    #     try:
    # print('-' * 10, 'stadegy', str(i), '-' * 10)

    printTradeAnalysis(back[i].analyzers.ta.get_analysis())
